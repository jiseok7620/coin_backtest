...
class Thread1(QThread):
    ...
    def run(self): # 과거 기간별 데이터 가져오기
        try: 
            ...
            minit_arr = ['1m', '5m', '15m', '30m', '1h', '4h', '1d']
            ...
            for minit in minit_arr :
                if os.path.isfile('D:/coinInvest/cointest/'+jongname[0:3]+'_'+minit+'.xlsx') :
                    ...
                    if minit == '1m' : 
                        ...
                        ddtm_newstart = start_time_bf - timedelta(hours=150)
                        ddtm_newend = end_time_bf
                    elif minit == '1h' :
...
class MyWindow(QMainWindow, form_class):
    ...
    def __init__(self):
        ...
        self.df_30m = pd.DataFrame() # 30분 데이터 저장
    ...
    def make_avgline(self, df, send):
        if send == 'main' :
            ...
            avg_120 = df['close'].rolling(window=120).mean()
            avg_200 = df['close'].rolling(window=200).mean()
            return avg_20, avg_60, avg_120, avg_200
    ...
    def make_separ(self, dataset, price, avgline):
        ...
        return ((dataset[price] / dataset[avgline]) - 100) * 100 
    ...
    ##----------------------------------------------------------------------------##                        
    ## --새창 열기 함수 모음-- ##
    def open_newwindow(self, dataset, dateset_5m, dateset_15m, dateset_30m, dateset_1h, dateset_4h, dataset_1d):
        self.hide() # 메인 윈도우 숨김
        self.second = main_second_cls(dataset, dateset_5m, dateset_15m, dateset_30m, dateset_1h, dateset_4h, dataset_1d)
        self.second.exec()
        self.show()
    ...
    def pushButton_testStart_click(self):
        try:
            ...
            # 3분, 5분, 15분, 30분, 1시간, 4시간 데이터 조회 후 가공하기
            ...
            dataset_30m = pd.read_excel('D:/coinInvest/cointest/'+jongname[0:3]+'_30m.xlsx', engine='openpyxl')
            ...
            self.df_30m = dataset_30m
            self.df_1h = dataset_1h
            self.df_4h = dataset_4h
            
            # 데이터에 보조지표 추가하기(이동평균선)
            self.df_1m['20avg'], self.df_1m['60avg'], self.df_1m['120avg'], self.df_1m['200avg'] = self.make_avgline(self.df_1m, 'sub') # 이평 추가
            self.df_5m['20avg'], self.df_5m['60avg'], self.df_5m['120avg'], self.df_5m['200avg'] = self.make_avgline(self.df_5m, 'sub') # 이평 추가
            self.df_15m['20avg'], self.df_15m['60avg'], self.df_15m['120avg'], self.df_15m['200avg'] = self.make_avgline(self.df_15m, 'sub') #이평 추가
            self.df_30m['20avg'], self.df_30m['60avg'], self.df_30m['120avg'], self.df_30m['200avg'] = self.make_avgline(self.df_30m, 'sub') # 이평 추가
            self.df_1h['20avg'], self.df_1h['60avg'], self.df_1h['120avg'], self.df_1h['200avg'] = self.make_avgline(self.df_1h, 'sub') # 이평 추가
            self.df_4h['20avg'], self.df_4h['60avg'], self.df_4h['120avg'], self.df_4h['200avg'] = self.make_avgline(self.df_4h, 'sub') # 이평 추가
            self.df_1d['20avg'], self.df_1d['60avg'], self.df_1d['120avg'], self.df_1d['200avg'] = self.make_avgline(self.df_1d, 'sub') # 이평 추가
            
            # 데이터에 보조지표 추가하기(볼린저밴드)
            self.df_1m['bb_up'], self.df_1m['bb_down'] = self.make_BB(self.df_1m)
            self.df_5m['bb_up'], self.df_5m['bb_down'] = self.make_BB(self.df_5m)
            self.df_15m['bb_up'], self.df_15m['bb_down'] = self.make_BB(self.df_15m)
            self.df_30m['bb_up'], self.df_30m['bb_down'] = self.make_BB(self.df_30m)
            self.df_1h['bb_up'], self.df_1h['bb_down'] = self.make_BB(self.df_1h)
            self.df_4h['bb_up'], self.df_4h['bb_down'] = self.make_BB(self.df_4h)
            self.df_1d['bb_up'], self.df_1d['bb_down'] = self.make_BB(self.df_1d)
            
            # 데이터에 보조지표 추가하기(이격도)
            if self.combo_separ.currentText() == '20avg' :
                self.df_1m['separ'] = self.make_separ(self.df_1m, 'close', '20avg')
                self.df_5m['separ'] = self.make_separ(self.df_5m, 'close', '20avg')
                self.df_15m['separ'] = self.make_separ(self.df_15m, 'close', '20avg')
                self.df_30m['separ'] = self.make_separ(self.df_30m, 'close', '20avg')
                self.df_1h['separ'] = self.make_separ(self.df_1h, 'close', '20avg')
                self.df_4h['separ'] = self.make_separ(self.df_4h, 'close', '20avg')
                self.df_1d['separ'] = self.make_separ(self.df_1d, 'close', '20avg')
            elif self.combo_separ.currentText() == '60avg' :
                self.df_1m['separ'] = self.make_separ(self.df_1m, 'close', '60avg')
                self.df_5m['separ'] = self.make_separ(self.df_5m, 'close', '60avg')
                self.df_15m['separ'] = self.make_separ(self.df_15m, 'close', '60avg')
                self.df_30m['separ'] = self.make_separ(self.df_30m, 'close', '60avg')
                self.df_1h['separ'] = self.make_separ(self.df_1h, 'close', '60avg')
                self.df_4h['separ'] = self.make_separ(self.df_4h, 'close', '60avg')
                self.df_1d['separ'] = self.make_separ(self.df_1d, 'close', '60avg')
            elif self.combo_separ.currentText() == '120avg' :
                self.df_1m['separ'] = self.make_separ(self.df_1m, 'close', '120avg')
                self.df_5m['separ'] = self.make_separ(self.df_5m, 'close', '120avg')
                self.df_15m['separ'] = self.make_separ(self.df_15m, 'close', '120avg')
                self.df_30m['separ'] = self.make_separ(self.df_30m, 'close', '120avg')
                self.df_1h['separ'] = self.make_separ(self.df_1h, 'close', '120avg')
                self.df_4h['separ'] = self.make_separ(self.df_4h, 'close', '120avg')
                self.df_1d['separ'] = self.make_separ(self.df_1d, 'close', '120avg')
                ...
                self.df_30m['separ'] = self.make_separ(self.df_30m, 'close', '200avg')
            ...
            if self.combo_strength.currentText() == 'RSI' :
                ...
                self.df_15m['strength'] = self.make_rsi(self.df_15m, 14)
                self.df_30m['strength'] = self.make_rsi(self.df_30m, 14)
                self.df_1h['strength'] = self.make_rsi(self.df_1h, 14)
                self.df_4h['strength'] = self.make_rsi(self.df_4h, 14)
            ...
            self.df_sub = test_df # 메인에서 쓸 1분데이터
            self.df_1m = self.df_1m[stdd_ind2:eddd_ind] # main_second 에서 쓸 1분데이터
    ...
    def push_start_click(self):
        try:
            ...
            if self.combo_all.currentText() == '자동' :
                ...
                # 강도 지표 추가
                if self.combo_strength.currentText() == 'RSI' :
                    self.df['strength'] = self.make_rsi(self.df, 14)
            ...
            # 수동이라면    
                ...
                self.open_newwindow(self.df_1m, self.df_5m, self.df_15m, self.df_30m, self.df_1h, self.df_4h, self.df_1d)
